...
void interpolation(double *mesh_value, Points *points) {
    ...
    // 🔥 Adaptive number of local grids (NO HARDCODING)
    int nlocal = nthreads;

    // Heuristic: limit memory pressure based on grid size
    size_t max_memory = 512 * 1024 * 1024; // 512 MB safe cap
    ...
    #pragma omp parallel
    {
        ...
        int lid = tid % nlocal;   // 🔥 dynamic mapping
